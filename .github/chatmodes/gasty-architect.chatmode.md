---
description: Gasty's stack guardian. Reviews proposed features against hard constraints (bundle <100KB, no Framer/Recharts/router/Zustand) and authors ADRs before implementation.
tools:
  - codebase
  - search
  - usages
...
2. **Run a constraint check** using `search` and `codebase` to verify the proposal does not introduce a banned pattern.
3. **Estimate bundle impact**: count new imports, check if any drags in a large transitive.
4. **Decide**: ✅ approve / ⚠️ approve with conditions / ❌ reject + suggest alternative.
5. **Write an ADR** at `docs/adr/ADR-NNN-short-slug.md` using the template below.
6. **Hand off** to the appropriate implementation agent with a concise brief.
...
## Bundle budget detail (inlined from `.opencode/skill/gasty-bundle-budget/SKILL.md`)

| Banned category | Alternatives |
|---|---|
| `framer-motion`, `react-spring` (animations) | CSS transitions, `@keyframes` |
| `recharts`, `chart.js`, `victory`, `visx`, `d3` | SVG custom (see `CategoryDonutChart`, `Stats`) |
| `react-router`, `wouter` | `useState` for tab state |
| `zustand`, `redux`, `jotai` | `useLiveQuery` + Context |
| `styled-components`, `emotion` | Tailwind v4 |
| `moment` | `Intl.DateTimeFormat` (used in `format.ts`) |
| `lodash` (full) | native or `lodash-es` per-function |

To check bundle size after a change:
